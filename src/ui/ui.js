...
import { getItemDefinition } from "../content/items/items.js";
...
export function setupUi({ outputElement, inputElement, enterButton } = {}) {
  output = outputElement ?? null;
  input = inputElement ?? null;
  enter = enterButton ?? null;
...
function buildRpgOutputHtml() {
  ...
  const hiredAllies = merchants.filter((merchant) => merchant.hp > 0 && merchant.isHired).length + companions.filter((companion) => companion.hp > 0 && companion.state !== "dead").length;
...
Acciones:
Atacar, Conjurar, Usar objeto, Esquivar. La inspección ocurre automáticamente al pasar el mouse por el tablero.

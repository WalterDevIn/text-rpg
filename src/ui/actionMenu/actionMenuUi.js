...
import { player, hasSpellSlotFor, getSpellSlotLine, isPlayerSpellAvailable } from "../../content/creatures/player.js";
import {
  ...
  getLivingEnemies,
} from "../../engine/rules/dndCombatRules.js";
import { canPlaceEntityAt, getDistance, getSurfaceDistance, normalizeVector } from "../../engine/world/collision.js";
import { ActionType, GameMode, gameState } from "../../engine/state/gameState.js";
import {
  DUNGEON_DOOR_CHAR,
  DUNGEON_FLOOR_CHAR,
  DUNGEON_STAIRS_CHAR,
...
import { worldToScreen, screenToWorld } from "../../render/renderer.js";
import { getForcecageTiles, getWallOfForceTiles } from "../../content/spells/spellGeometry.js";
import {
  chests,
  groundItems,
  findChestAtPoint,
  ...
  lootChestAtPoint,
  pickupTorchAtPoint,
} from "../../content/items/inventory.js";
import { getItemDefinition, ItemId, ItemTargeting, ItemType } from "../../content/items/items.js";
import { getSpellDefinition, getSpellDefinitionByActionType } from "../../content/spells/spellDefinitions.js";
import {
  buyMerchantItem,
  hireMerchant,
  ...
  merchants,
} from "../../content/creatures/merchant.js";
import { companions } from "../../content/companions/companions.js";
import { requestCreaturePortrait } from "../hud/portraits.js";
import { getLongRestStatus } from "../../engine/rules/restRules.js";
import { getDancingLightAtPoint, moveDancingLight } from "../../engine/projectiles/projectileSystem.js";
...
export function setupActionMenuUi({ layerElement, canvasElement, rootElement }) {
  ...
  layer.addEventListener("pointerleave", handleBoardPointerLeave);
...
function renderActionMenu() {
  ...
  const mainButtons = [
    ...
    },
    {
      ...
      action: selectLongRest,
      selected: gameState.selectedAction === ActionType.LONG_REST,
    },
  ];
  ...
  if (submenuType === "shocking_grasp_targets") {
    renderTargetingHelp({
      text: `Elegí un enemigo adyacente. Rango: ${SHOCKING_GRASP_RANGE_FEET} pies.`,
      center: playerScreenPosition,
    });
    renderTargetIndicators(getVisibleTargets(getEnemiesInShockingGraspRange()), {
      variant: "lightning",
      onSelect: selectShockingGraspTarget,
    });
  }
  ...
  if (submenuType === "dancing_lights_point") {
    renderTargetingHelp({
      text: `Tocá un punto para colocar las luces o una criatura marcada para que la sigan. Rango: ${DANCING_LIGHTS_RANGE_FEET} pies.`,
      center: playerScreenPosition,
    });
    renderTargetIndicators(getVisibleTargets(getDancingLightsTargets()), {
      variant: "dancing-light",
      onSelect: selectDancingLightsTarget,
    });
    renderCirclePreview(gameState.selectedBoardPoint ?? gameState.mouseWorldPoint ?? player, TILE_SIZE * 0.75, true, "#76c7ff");
  }

  if (submenuType === "dancing_light_reposition") {
    renderTargetingHelp({
      text: "Tocá un punto para reubicar la luz danzante elegida.",
      center: playerScreenPosition,
    });
  }
  ...
  if (submenuType === "vortex_warp_targets") {
    renderTargetingHelp({
      text: `Elegí la criatura que querés mover. Rango: ${VORTEX_WARP_RANGE_FEET} pies.`,
      center: playerScreenPosition,
    });
    renderTargetIndicators(getVisibleTargets(getVortexWarpTargets()), {
      variant: "vortex",
  ...
  if (submenuType === "vortex_warp_point") {
    renderTargetingHelp({
      text: `Tocá el destino para mover a la criatura. Rango desde ti: ${VORTEX_WARP_RANGE_FEET} pies.`,
      center: playerScreenPosition,
    });
...
function isCastActionSelected() {
  return (
    ...
    gameState.selectedAction === ActionType.CAST_DANCING_LIGHTS ||
    gameState.selectedAction === ActionType.CAST_SHOCKING_GRASP ||
    gameState.selectedAction === ActionType.CAST_FALSE_LIFE ||
    gameState.selectedAction === ActionType.CAST_EXPEDITIOUS_RETREAT ||
...
function selectLongRest() {
  ...
  if (!status.allowed) {
    ...
    gameState.selectedAction = null;
    clearAllSelections();
    gameState.message = status.reason;
    renderActionMenu();
    return;
  }

  submenuType = null;
  gameState.selectedAction = ActionType.LONG_REST;
  clearAllSelections();
  gameState.message = "Descanso largo preparado. Soltá F para descansar en esta sala segura.";
  renderActionMenu();
}
...
function renderSpellSubmenu(center) {
  ...
  const spells = [
    ...
    },
    {
      name: "Saeta de fuego",
      level: 0,
      description: `Truco. Dispara una x ígnea en línea recta. Alcance ${FIRE_BOLT_RANGE_FEET} pies. Daño 1d10. Casteo 3s; luego cooldown 3s.`,
...

    {
      ...
      level: 0,
      description: `Truco. Crea 4 orbes de luz durante 60s a ${DANCING_LIGHTS_RANGE_FEET} pies.`,
    ...
    },
    {
      ...
      level: 0,
      description: `Truco instantáneo de toque. Daño 1d8 e inactiva al enemigo 6s. Rango ${SHOCKING_GRASP_RANGE_FEET} pies.`,
    ...
    },
    {
      name: "Escudo",
      level: 1,
      description: "Reacción instantánea; barrera 6s; bloquea daño hasta los PG máximos del objetivo.",
    ...
    },
    {
      name: "Vida falsa",
      level: 1,
      description: "Otorga 1d4+4 PG temporales a una criatura visible. Casteo 3s.",
    ...
    },
    {
      ...
      level: 1,
      description: "Conjuro instantáneo. Aumenta tu velocidad durante 10 minutos.",
    ...
    },
    {
      name: "Vortex Warp",
      level: 2,
      description: `Mueve una criatura a un punto elegido dentro de ${VORTEX_WARP_RANGE_FEET} pies.`,
  ...
  const availableSpells = spells.filter((spell) => isPlayerSpellAvailable(spell.actionType));
  ...
  renderPaginatedList({
    ...
    renderItem: (spell) => {
      ...
      const definition = getSpellDefinitionByActionType(spell.actionType);
      const concentrationText = definition?.concentration ? " · concentración" : "";
      const levelText = spell.level === 0 ? "truco" : `nivel ${spell.level ?? "?"}`;
      button.textContent = `${spell.name} · ${levelText}${concentrationText}${hasSlot ? "" : " · sin espacios"}`;
      button.title = spell.level === 0
        ? `${spell.description}${definition?.concentration ? "\nRequiere concentración." : ""}\nNo consume espacios de conjuro.`
...
function getMerchantDialogueText(merchant) {
  ...
  return [
    ...
    `${formatHoverHp(merchant)}. Actitud: ${merchant.isHired ? "acompañante contratada" : "no hostil"}.`,
  ].join("\n");
}
...
function chooseUsableItem(item) {
  ...
  if (definition?.spellId) {
    chooseSpellScroll(item, definition);
    return;
  }
...
function chooseSpellScroll(item, definition) {
  const spell = getSpellDefinition(definition.spellId);
  ...
  if (!spell) {
    submenuType = "items";
    gameState.message = `${definition.name} no tiene definición de conjuro válida.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "creature") {
    ...
    gameState.message = targets.length > 0
      ? `Pergamino elegido: ${spell.name}. Tocá la criatura objetivo.`
      : `Pergamino elegido: ${spell.name}. No hay criaturas visibles en rango.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "enemy") {
    ...
    gameState.message = targets.length > 0
      ? `Pergamino elegido: ${spell.name}. Tocá un enemigo marcado o soltá F para usarlo contra ${targets[0].name}.`
      : `Pergamino elegido: ${spell.name}. No hay enemigos en rango.`;
    renderActionMenu();
    return;
  }

  if (spell.targeting === "casting_enemy") {
    submenuType = "counterspell_targets";
    const targets = getVisibleTargets(getCounterspellTargets());
    gameState.selectedTargetId = targets[0]?.id ?? null;
    gameState.message = targets.length > 0
      ? `Pergamino elegido: ${spell.name}. Tocá el casteador enemigo.`
      : `Pergamino elegido: ${spell.name}. No hay casteos enemigos visibles.`;
    renderActionMenu();
    return;
  }

  if (spell.targeting === "enemy_touch") {
    submenuType = "shocking_grasp_targets";
    const targets = getVisibleTargets(getEnemiesInShockingGraspRange());
    gameState.selectedTargetId = targets[0]?.id ?? null;
    gameState.message = targets.length > 0
      ? `Pergamino elegido: ${spell.name}. Tocá un enemigo adyacente.`
      : `Pergamino elegido: ${spell.name}. No hay enemigos a ${SHOCKING_GRASP_RANGE_FEET} pies.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "fire_bolt_direction") {
    ...
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá un punto para disparar en esa dirección.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "direction") {
    ...
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá un punto para orientar el cono.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "point") {
    ...
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá el punto objetivo.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "fireball_point") {
    ...
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá el punto donde explotará.`;
    renderActionMenu();
    return;
  }

  if (spell.targeting === "dancing_lights_point") {
    submenuType = "dancing_lights_point";
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá un punto o una criatura para que las luces la sigan.`;
    renderActionMenu();
    return;
  }
  ...
  if (spell.targeting === "chromatic_orb_point") {
    ...
    gameState.message = `Pergamino elegido: ${spell.name}. Tocá el punto inicial del orbe.`;
    renderActionMenu();
    return;
  }

  if (spell.targeting === "creature_then_point") {
    submenuType = "vortex_warp_targets";
    const targets = getVisibleTargets(getVortexWarpTargets());
    gameState.selectedTargetId = targets[0]?.id ?? null;
    gameState.message = targets.length > 0
      ? `Pergamino elegido: ${spell.name}. Tocá la criatura que querés mover.`
      : `Pergamino elegido: ${spell.name}. No hay criaturas en rango.`;
    renderActionMenu();
    return;
  }

  submenuType = "items";
  gameState.message = `Pergamino elegido: ${spell.name}. Soltá F para usarlo.`;
  renderActionMenu();
}

function isUsingSpellScroll() {
  const item = getInventoryItemByInstanceId(gameState.selectedInventoryItemId);
  const definition = getItemDefinition(item);
  return Boolean(definition?.spellId);
}

function setSelectedActionForSpellOrScroll(actionType) {
  gameState.selectedAction = isUsingSpellScroll() ? ActionType.USE_ITEM : actionType;
}
...
function chooseSpell(spell) {
  ...
  if (spell.targeting === "creature") {
    ...
    gameState.selectedTargetId = targets[0]?.id ?? "player";
    gameState.message = targets.length > 0
      ? `Conjuro elegido: ${spell.name}. Tocá la criatura objetivo.`
  ...
  if (spell.targeting === "enemy_touch") {
    ...
    gameState.selectedTargetId = targets[0]?.id ?? null;
    gameState.message = targets.length > 0
      ? `Conjuro elegido: ${spell.name}. Tocá un enemigo adyacente.`
      : `Conjuro elegido: ${spell.name}. No hay enemigos a ${SHOCKING_GRASP_RANGE_FEET} pies.`;
    return;
  }
  ...
  if (spell.targeting === "dancing_lights_point") {
    ...
    gameState.selectedAction = spell.actionType;
    clearAllSelections();
    gameState.message = `Conjuro elegido: ${spell.name}. Tocá un punto o una criatura para que las luces la sigan.`;
    return;
  }
  ...
  if (spell.targeting === "creature_then_point") {
    ...
    gameState.selectedTargetId = targets[0]?.id ?? null;
    gameState.message = targets.length > 0
      ? `Conjuro elegido: ${spell.name}. Tocá la criatura que querés mover.`
      : `Conjuro elegido: ${spell.name}. No hay criaturas en rango.`;
    return;
  }
...
function renderTargetIndicators(targets, { variant, onSelect }) {
  ...
  for (const target of targets) {
    ...
    indicator.title = `${target.name} | ${formatHoverHp(target)}`;
...
function getIndicatorBorder(variant) {
  ...
  if (variant === "lightning") return "2px solid #66ccff";
  if (variant === "dancing-light") return "2px solid #76c7ff";
...
function getIndicatorShadow(variant) {
  ...
  if (variant === "dancing-light") return "0 0 12px rgba(118, 199, 255, 0.85)";
...
function selectCureWoundsTarget(target) {
  const itemId = gameState.selectedInventoryItemId;
  const currentSpellAction = gameState.selectedAction;
  gameState.selectedAction = itemId ? ActionType.USE_ITEM : (currentSpellAction ?? ActionType.CAST_CURE_WOUNDS);
  gameState.selectedTargetId = target.id;
  clearNonTargetSelections();
  gameState.selectedInventoryItemId = itemId ?? null;
  gameState.message = `Objetivo elegido: ${target.name}. Soltá F para ${itemId ? "leer el pergamino" : "conjurar"}.`;
  renderActionMenu();
}

function selectMagicMissileTarget(target) {
  const itemId = gameState.selectedInventoryItemId;
  gameState.selectedAction = itemId ? ActionType.USE_ITEM : ActionType.CAST_MAGIC_MISSILE;
  gameState.selectedTargetId = target.id;
  clearNonTargetSelections();
  gameState.selectedInventoryItemId = itemId ?? null;
  gameState.message = `Objetivo elegido: ${target.name}. Soltá F para ${itemId ? "leer el pergamino" : "conjurar Misil mágico"}.`;
  renderActionMenu();
}


function selectShockingGraspTarget(target) {
  const itemId = gameState.selectedInventoryItemId;
  gameState.selectedAction = itemId ? ActionType.USE_ITEM : ActionType.CAST_SHOCKING_GRASP;
  gameState.selectedTargetId = target.id;
  clearNonTargetSelections();
  gameState.selectedInventoryItemId = itemId ?? null;
  gameState.message = `Objetivo elegido para Contacto electrizante: ${target.name}. Soltá F para ${itemId ? "leer el pergamino" : "conjurar"}.`;
  renderActionMenu();
}

function selectDancingLightsPoint(boardPoint) {
  setSelectedActionForSpellOrScroll(ActionType.CAST_DANCING_LIGHTS);
  gameState.selectedTargetId = null;
  gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
  gameState.selectedDodgeDirection = null;
  gameState.selectedAimDirection = null;
  gameState.selectedBoardPoint = boardPoint;

  if (getDistance(player, boardPoint) > DANCING_LIGHTS_RANGE_PIXELS) {
    gameState.message = "Punto elegido, pero está fuera del rango de Luces danzantes.";
  } else {
    gameState.message = "Punto elegido para Luces danzantes. Soltá F para empezar el casteo.";
  }

  renderActionMenu();
}

function selectDancingLightsTarget(target) {
  setSelectedActionForSpellOrScroll(ActionType.CAST_DANCING_LIGHTS);
  gameState.selectedTargetId = target.id;
  gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
  gameState.selectedDodgeDirection = null;
  gameState.selectedAimDirection = null;
  gameState.selectedBoardPoint = { x: target.x, y: target.y };
  gameState.message = `Objetivo elegido para Luces danzantes: ${target.name}. Los orbes lo seguirán. Soltá F para empezar el casteo.`;
  renderActionMenu();
}

function selectCounterspellTarget(target) {
  const itemId = gameState.selectedInventoryItemId;
  gameState.selectedAction = itemId ? ActionType.USE_ITEM : ActionType.CAST_COUNTERSPELL;
  gameState.selectedTargetId = target.id;
  clearNonTargetSelections();
  gameState.selectedInventoryItemId = itemId ?? null;
  gameState.message = `Counterspell preparado contra ${target.name}. Soltá F para ${itemId ? "leer el pergamino" : "interrumpirlo como reacción"}.`;
  renderActionMenu();
}

function selectVortexWarpTarget(target) {
  const itemId = gameState.selectedInventoryItemId;
  gameState.selectedAction = itemId ? ActionType.USE_ITEM : ActionType.CAST_VORTEX_WARP;
  gameState.selectedTargetId = target.id;
  gameState.selectedBoardPoint = null;
  gameState.selectedInventoryItemId = itemId ?? null;
  gameState.selectedDodgeDirection = null;
...
function selectFireballPoint(boardPoint) {
  setSelectedActionForSpellOrScroll(ActionType.CAST_FIREBALL);
  gameState.selectedTargetId = null;
  gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
  gameState.selectedDodgeDirection = null;
  gameState.selectedAimDirection = null;
...
function getCureWoundsTargets() {
  const creatures = [
    ...
    ...getVisibleCompanions().map((companion) => createCreatureTarget(companion, `companion:${companion.id}`)),
  ];
...
function getVortexWarpTargets() {
  return [
    ...getLivingEnemies().map((enemy) => createCreatureTarget(enemy, `enemy:${enemy.id}`)),
    ...getVisibleMerchants().filter((merchant) => merchant.hp > 0).map((merchant) => createCreatureTarget(merchant, `merchant:${merchant.id}`)),
    ...getVisibleCompanions().map((companion) => createCreatureTarget(companion, `companion:${companion.id}`)),
  ].filter((target) => getSurfaceDistance(player, target.entity) <= VORTEX_WARP_RANGE_PIXELS);
}

function getDancingLightsTargets() {
  return [
    createCreatureTarget(player, "player"),
    ...getLivingEnemies().map((enemy) => createCreatureTarget(enemy, `enemy:${enemy.id}`)),
    ...getVisibleMerchants().filter((merchant) => merchant.hp > 0).map((merchant) => createCreatureTarget(merchant, `merchant:${merchant.id}`)),
    ...getVisibleCompanions().map((companion) => createCreatureTarget(companion, `companion:${companion.id}`)),
  ].filter((target) => getDistance(player, target.entity) <= DANCING_LIGHTS_RANGE_PIXELS);
}

function getEnemiesInShockingGraspRange() {
  return getLivingEnemies().filter((enemy) => {
    return getSurfaceDistance(player, enemy) <= SHOCKING_GRASP_RANGE_PIXELS && isEntityVisibleToPlayer(enemy, player);
  });
}
...
function renderHoverInspectionPreview() {
...
function updateHoverInspection(boardPoint) {
...
function buildAnalysisInfo(boardPoint, alreadyFoundCreature = null) {
  ...
  if (tile === SURFACE_TREE_CHAR) {
    return "ANÁLISIS: árbol\nBloque marrón sólido. Forma parte del bosque y bloquea movimiento.";
  }

  if (tile === DUNGEON_WALL_CHAR) {
    return isSurfaceLayer()
      ? "ANÁLISIS: entrada de mazmorra\nMampostería antigua entre los árboles. Bloquea movimiento."
...
function findVisibleCreatureAtPoint(point) {
  const candidates = [player, ...getLivingEnemies(), ...getVisibleMerchants(), ...getVisibleCompanions()];
...
function getVisibleCompanions() {
  return companions.filter((companion) => companion.hp > 0 && companion.state !== "dead" && isEntityVisibleToPlayer(companion, player));
}
...
function getTemporaryHp(creature) {
  return Math.max(0, Math.floor(creature?.temporaryHp ?? 0));
}

function getCurrentHp(creature) {
  return Math.max(0, Math.floor(creature?.hp ?? 0));
}

function getMaxHp(creature) {
  return Math.max(1, Math.floor(creature?.maxHp ?? 1));
}

function getEffectiveHp(creature) {
  return getCurrentHp(creature) + getTemporaryHp(creature);
}

function formatHoverHp(creature) {
  const hp = getCurrentHp(creature);
  const maxHp = getMaxHp(creature);
  const temporaryHp = getTemporaryHp(creature);

  if (temporaryHp <= 0) {
    return `${hp}/${maxHp} PG`;
  }

  return `${hp}+${temporaryHp}/${maxHp} PG`;
}

function formatHoverHpLine(creature) {
  const hp = getCurrentHp(creature);
  const maxHp = getMaxHp(creature);
  const temporaryHp = getTemporaryHp(creature);
  ...
  return `PG: ${hp}+${temporaryHp}/${maxHp} (${getEffectiveHp(creature)} efectivos; ${temporaryHp} temporales por encima del máximo).`;
}


function formatPlayerConcentrationSuffix() {
  const concentration = player.concentration;
  if (!concentration?.spell) {
    return "";
  }

  const definition = getSpellDefinitionByActionType(concentration.spell);
  const name = concentration.name ?? definition?.name ?? "conjuro";
  return ` · concentración: ${name}`;
}
...
function describeCreature(creature) {
  if (creature === player) {
    return [
      ...
      `Nombre: ${player.name}.`,
      `Clase: ${player.className ?? "?"} nivel ${player.level ?? 1}${formatPlayerConcentrationSuffix()}.`,
      formatHoverHpLine(player),
      `Oro: ${player.gold} po.`,
  ...
  if (creature.kind === "centipede_part") {
    ...
    return [
      ...
      `Actitud: ${hostility}.`,
      `Rol anatómico: ${role}.`,
      formatHoverHpLine(creature).replace("PG:", "PG de esta parte:"),
...
function handleBoardPointerMove(event) {
  ...
  updateHoverInspection(boardPoint);
...
function updateLiveTargetingPreviewFromBoardPoint(boardPoint) {
  if (submenuType === "burning_hands_aim") {
    ...
    setSelectedActionForSpellOrScroll(ActionType.CAST_BURNING_HANDS);
    gameState.selectedTargetId = null;
    gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
    gameState.selectedDodgeDirection = null;
  ...
  if (submenuType === "fire_bolt_aim") {
    ...
    setSelectedActionForSpellOrScroll(ActionType.CAST_FIRE_BOLT);
    gameState.selectedTargetId = null;
    gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
    gameState.selectedDodgeDirection = null;
    gameState.selectedAimDirection = direction;
  ...
  if (submenuType === "dancing_lights_point") {
    // La selección de criatura para Luces danzantes debe ser explícita:
    // sólo cambia a modo seguimiento al tocar un indicador de criatura.
    // Mover el mouse sobre una criatura no debe robar la selección del punto.
  ...
  if (submenuType === "chromatic_orb_point") {
    setSelectedActionForSpellOrScroll(ActionType.CAST_CHROMATIC_ORB);
    gameState.selectedTargetId = null;
    gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
    gameState.selectedDodgeDirection = null;
    gameState.selectedAimDirection = null;
...
function handleBoardPointerDown(event) {
  ...
  if (!isBoardTargetingActive()) {
    const clickedLight = getDancingLightAtPoint(boardPoint);
    if (clickedLight) {
      selectedDancingLight = clickedLight;
      submenuType = "dancing_light_reposition";
      gameState.message = "Luz danzante elegida. Tocá el nuevo punto para reubicarla.";
      renderActionMenu();
      event.preventDefault();
      event.stopPropagation();
      return;
  ...
  if (submenuType === "shocking_grasp_targets") {
    selectClosestBoardTarget(boardPoint, getVisibleTargets(getEnemiesInShockingGraspRange()), selectShockingGraspTarget);
    return;
  }
  ...
  if (submenuType === "vortex_warp_targets") {
    selectClosestBoardTarget(boardPoint, getVisibleTargets(getVortexWarpTargets()), selectVortexWarpTarget);
    return;
  }
  ...
  if (submenuType === "dancing_lights_point") {
    const targets = getVisibleTargets(getDancingLightsTargets());
    const closestTarget = findClosestClickedTarget(boardPoint, targets);

    if (closestTarget) {
      selectDancingLightsTarget(closestTarget);
      return;
    }

    selectDancingLightsPoint(boardPoint);
    return;
  }

  if (submenuType === "dancing_light_reposition") {
    const light = selectedDancingLight;
    if (light && moveDancingLight(light, boardPoint)) {
      gameState.message = "Reubicas una luz danzante.";
    }
    selectedDancingLight = null;
    submenuType = null;
    renderActionMenu();
    return;
  }
...
function selectWallPoint(boardPoint) {
  setSelectedActionForSpellOrScroll(ActionType.CAST_WALL_OF_FORCE);
  gameState.selectedTargetId = null;
  gameState.selectedInventoryItemId = gameState.selectedInventoryItemId ?? null;
  gameState.selectedDodgeDirection = null;
  gameState.selectedAimDirection = null;

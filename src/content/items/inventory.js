...
import { enemies, killEnemy } from "../creatures/enemy.js";
import { rollDie } from "../../engine/rules/dice.js";
import { addTemporaryWall, addTorch, getTorchAt, isStaticWallAt, removeTorch } from "../../engine/world/map.js";
...
import {
  ...
  ItemTargeting,
  ItemType,
  getAllSpellScrollItemIds,
} from "./items.js";
import { resolveSpell } from "../spells/spellCasting.js";
import { getForcecageTiles } from "../spells/spellGeometry.js";

export const groundItems = [];
export const chests = [];
let nextChestId = 1;

const spellScrollLootEntries = getAllSpellScrollItemIds().map((definitionId) => ({ definitionId, weight: 5 }));
...
export function isSpellScrollItem(item) {
  const definition = getItemDefinition(item);
  return definition?.type === ItemType.SCROLL && Boolean(definition.spellId && definition.actionType);
}

export function getSpellScrollDefinition(item) {
  const definition = getItemDefinition(item);
  return isSpellScrollItem(item) ? definition : null;
}
...
export function useSelectedInventoryItem(options = {}) {
  ...
  if (definition.spellId && definition.actionType) {
    useSpellScroll(item, definition, options);
    return;
  }
...
function useSpellScroll(item, definition, options = {}) {
  if (!definition?.actionType) {
    gameState.message = "Ese pergamino no tiene conjuro asociado.";
    return;
  }

  resolveSpell(definition.actionType, {
    targetId: options.targetId,
    boardPoint: options.boardPoint,
    aimDirection: options.aimDirection,
    caster: player,
  });

  consumeInventoryItem(item.instanceId);
  gameState.message = `${gameState.message}\nEl ${definition.name} se consume.`;
}
